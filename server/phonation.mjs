...
async function readContext({dataRoot,enabled=process.env.PHONATION_MEASUREMENT_ENABLED==='1',now=Date.now(),maxAgeMs=300000}){
 const result={capabilityVersion:'optional-phonation-runtime-1.0.0',baselineScoring:'unchanged',measurement:capability('disabled','Optional phonation measurement is disabled'),inference:capability('disabled','This measurement-only path does not infer a source model; source experiments have a separate capability'),coaching:capability('disabled','No current verified phonation evidence'),latest:null,running:false};
...
export async function readPhonationContext(options){
 const context=await readContext(options);if(!options.compact)return context;
 const {latest,descriptors,...bounded}=context;
 return {...bounded,latestAnalysis:latest?{id:latest.id,createdAt:latest.createdAt,evidenceAt:latest.evidenceAt,status:latest.status}:null,...(descriptors?{descriptors:descriptors.observations.slice(0,4).map(row=>({observationId:row.observationId,extractorVersion:row.extractorVersion,configurationVersion:row.configurationVersion,evidenceAt:row.evidenceAt,descriptors:row.descriptors}))}:{})};
}
