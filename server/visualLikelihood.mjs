...
export function createVisualLikelihoodRoutes({repo,dataRoot,json}){
 ...
 return async(req,res,url)=>{
  ...
  // Reserve before any disk, body or worker await. Two POST handlers must never
  // both observe idle and overwrite the durable operation pointer.
  const mutation=req.method==='POST';
  if(mutation&&(reserved||busy)){json(res,409,{error:'A visual experiment is already running.'});return true;}
  if(mutation)reserved=true;
  try{
   ...
   if(req.method==='GET'&&url.pathname.endsWith('/status')){
    ...
    json(res,200,{enabled:enabled(),busy:wasBusy||busy||reserved,captures,currentModelId:current?.snapshot?.model_id??null,
  ...
  finally{if(mutation)reserved=false;}
 };
}
