import teachingCatalog from '../src/teaching/decisionCatalog.json' with {type:'json'};
...
export function createAstraRoutes({dataRoot,json,provider,fetchImpl=fetch,callBudget=6}) {
  ...
  async function current(){let run;try{run=await read(resolve(dataRoot,'science-current.json'));}catch(error){if(error.code==='ENOENT')throw failure('Import a voice capture and fit the scientific model before asking Astra');throw error;}if(run.status!=='succeeded'||!id(run.runId))throw failure('A completed voice fit is required');const summary=await read(resolve(dataRoot,'science-runs',run.runId,'summary.json'));if(!id(summary.sessionId))throw failure('Voice fit has no scientific session');return {runId:run.runId,summary,sessionId:summary.sessionId};}
  ...
  async function forecast(context,state,requestId){const pendingRequest=state.pending?.request,pendingDesign=pendingRequest?.parameters?.design_id;const owner=typeof pendingDesign==='string'?/^astra-forecast-([A-Za-z0-9_-]{1,80})$/.exec(pendingDesign)?.[1]:null;const owned=pendingRequest?.operation==='design_pcm'&&owner&&pendingRequest.parameters.target_observation_id===`astra-target-${owner}`;if(state.pending&&!owned)throw failure('A scientific job is already running');if(!state.snapshot)throw failure('A fitted hypothesis model is required');let active=Object.values(state.designs).filter(d=>d.data.model_id===state.snapshot.model_id&&['committed','unsupported'].includes(d.status)).at(-1);if(active&&!owned)return {state,design:active};
  ...
  return async(req,res,url)=>{
    ...
    try{
      ...
      try{
        ...
        try{const {readPhonationContext}=await import('./phonation.mjs');const optional=await readPhonationContext({dataRoot,compact:true});prompt.phonation={capabilityVersion:optional.capabilityVersion,baselineScoring:optional.baselineScoring,measurement:optional.measurement,inference:optional.inference,coaching:optional.coaching,...(optional.descriptors?{descriptors:optional.descriptors}:{})};}
        ...
        if(teachingEnabled)prompt.teaching={version:teachingCatalog.version,demonstrations:teachingCatalog.demonstrations,scope:'Choose an optional educational mechanism relevant to this supported sustained-vowel task. A catalog cue is not a new authorized action; use cueId null if it changes the committed task. Rest requires null demonstrationId and cueId.'};
        const decisionSchema=teachingEnabled?{...schema,required:[...schema.required,'demonstrationId','cueId'],properties:{...schema.properties,demonstrationId:{type:['string','null'],enum:[null,...teachingCatalog.demonstrations.map(d=>d.id)]},cueId:{type:['string','null'],enum:[null,...new Set(teachingCatalog.demonstrations.flatMap(d=>d.cues.map(c=>c.id)))]}}}:schema;
        ...
        if(teachingEnabled){const demonstration=teachingCatalog.demonstrations.find(d=>d.id===decision.demonstrationId);if((decision.demonstrationId!==null&&!demonstration)||(decision.cueId!==null&&!demonstration?.cues.some(c=>c.id===decision.cueId))||(decision.action==='rest'&&(decision.demonstrationId!==null||decision.cueId!==null)))throw failure('Astra returned an unsupported teaching selection',502);}
