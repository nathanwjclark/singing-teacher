  ...
  return async(req,res,url)=>{
    ...
    try{
      ...
      try{
        ...
        try{const {readSourceInferenceContext}=await import('./sourceInference.mjs');const source=await readSourceInferenceContext({dataRoot});const fit=source.fit?.result;prompt.sourceInference={enabled:source.enabled,interpretation:source.interpretation,baselinePreserved:true,sourceModelId:source.sourceModelId,fitStatus:source.fit?.status,forecastStatus:source.forecast?.authoritativeStatus,hypotheses:fit?.joint?.candidates?.map(c=>({id:c.candidate_id,anatomy:c.anatomy,score:c.score,status:c.status,controls:c.predictions?.map(p=>p.controls)})),forecast:source.forecast?.current?source.forecast.result:null,score:source.score?.result||null};}
