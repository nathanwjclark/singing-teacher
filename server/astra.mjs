...
export function createAstraRoutes({dataRoot,json,provider,fetchImpl=fetch,callBudget=6}) {
  ...
  async function finish(context,receipt,state,path){
    ...
    }else await save(resolve(dataRoot,'science-runs',context.runId,'astra-rest.json'),{sessionId:context.sessionId,decisionId:receipt.requestId,createdAt:new Date().toISOString()});
  ...
  return async(req,res,url)=>{
    ...
    try{
      ...
      try{
        ...
        prompt.outcomes=(state.jobs||[]).filter(j=>j.request?.operation==='update_pcm'&&j.result).slice(-6).map(j=>({jobId:j.job_id,result:j.result}));
