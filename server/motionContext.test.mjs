...
const temporal=(overrides={})=>({status:'available',informationOverConstant:'present',settings:{penalties:[0,.1,1],maxLinkGapSeconds:.5,objectiveGapTolerance:.1,transitionInterpretation:'regularizer',uncertaintyInterpretation:'sensitivity sets',objective:'not copied'},
...
test('ambiguity sets, sampled segment boundaries and excluded-window reasons survive with explicit truncation',()=>{
 ...
 const t=result.temporal;assert.equal(t.informationOverConstant,'present');assert.equal(result.warnings[0].code,'no-information-over-constant');
 ...
 assert.deepEqual(t.excludedWindows.reasons,[{reason:'Unvoiced or invalid canonical window',count:20},{reason:'Measured pitch outside the one-semitone bank support',count:20}]);
...
test('readMotionContext reports unavailable instead of an oversized verified context',async t=>{
 ...
 assert.deepEqual(await readMotionContext({dataRoot:root,sessionId:'session-one',modelId:'model-one'}),{status:'unavailable',modelUpdated:false,reason:'No verified current motion audio analysis'});
});
