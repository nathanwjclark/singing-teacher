...
export function createAstraReviewRoutes({dataRoot,json,envFile,fetchApi=fetch,apiKey=()=>process.env.OPENAI_API_KEY}) {
  const file=resolve(dataRoot,'astra-review.json'); let pending=null,controller=null,generation=0,resettingCount=0,writeQueue=Promise.resolve();
  const save=(value,expected=generation)=>{const operation=writeQueue.catch(()=>{}).then(async()=>{if(expected!==generation)return;await mkdir(dataRoot,{recursive:true});const temp=file+'.tmp';await writeFile(temp,JSON.stringify(value,null,2),{mode:0o600});await rename(temp,file)});writeQueue=operation;return operation};
  ...
  async function execute(ctx,key,expected,signal){
    ...
    try{
      ...
      const latest=await context();if(expected!==generation||latest.captureKey!==ctx.captureKey||latest.runId!==ctx.runId||latest.outcomeId!==ctx.outcomeId)return;
      await save({...base,status:'complete',text:text.slice(0,12000),responseId:result.id,completedAt:new Date().toISOString()},expected);
    }catch(error){const latest=await context();if(expected===generation&&latest.captureKey===ctx.captureKey&&latest.runId===ctx.runId&&latest.outcomeId===ctx.outcomeId)await save({...base,status:'error',message:error.name==='TimeoutError'?'Astra review timed out. Retry.':error.message?.startsWith('OpenAI request failed')||error.message?.startsWith('Astra returned')?error.message:'Astra review could not connect. Check local API configuration and retry.'},expected)}
  }
  return async(req,res,url)=>{
    const resetting=url.pathname==='/api/astra-review/reset';
    if(url.pathname!=='/api/astra-review'&&!resetting)return false;
    ...
    if(resetting){
      ...
      resettingCount++;
      try{
        const expected=++generation;controller?.abort();controller=null;pending=null;
        const ctx=await context();
        const dismissed={schemaVersion:'astra-review-1',status:'dismissed',captureKey:ctx.captureKey,captureId:ctx.receipt?.captureId,resetAt:new Date().toISOString()};
        await save(dismissed,expected);json(res,200,{...dismissed,canReview:false});return true;
      }finally{resettingCount--}
    }
    const expected=generation;
    const {ctx,key,state,canReview}=await view();
    if(req.method==='POST'){
      if(resettingCount||expected!==generation){json(res,409,{status:'dismissed',canReview:false});return true}
      ...
      const task=execute(ctx,key,expected,controller.signal).catch(()=>{/* A local persistence failure must not crash the server. */}).finally(()=>{if(pending===task){pending=null;controller=null}});pending=task;
