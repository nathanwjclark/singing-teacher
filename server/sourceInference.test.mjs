...
test('compact bank retains bounded unavailable alternatives and conditional heldout lineage',async t=>{
 ...
 process.env.SCIENCE_URL='http://127.0.0.1:8766';process.env.SCIENCE_TOKEN='test';
 t.after(()=>{globalThis.fetch=previous;if(url===undefined)delete process.env.SCIENCE_URL;else process.env.SCIENCE_URL=url;if(token===undefined)delete process.env.SCIENCE_TOKEN;else process.env.SCIENCE_TOKEN=token;});
 const rows=Array.from({length:51},(_,i)=>({alternative_id:'joint:'+i,family:'joint',candidate_id:String(i),source_model:'two_mass',status:i?'available':'unavailable',reason:i?null:'unsupported-window',requested_f0_hz:180,simulated_f0_hz:i?171.5:null,record:i?{descriptors:{pitch:180}}:null}));
 ...
 await writeFile(join(dir,'source-score.json'),JSON.stringify({status:'succeeded',result:{baselineModelId:'model-one',status:'scored',forecast_sha256:'bank-hash',alternatives:rows.map((row,i)=>({...row,score:i?2:null,score_excluding_pitch:i?1.5:null,heldout_rank:i||null,heldout_rank_excluding_pitch:i||null})),ranking:['joint:1'],conditionalRanking:{rankingId:'rank-one',version:1,bankSha256:'bank-hash'},model_updated:false}}));
 ...
 assert.equal(compact.score.conditionalRanking.alternatives[0].discrepancy,null);
 assert.deepEqual([compact.forecast.bank.alternatives[1].sourceModel,compact.forecast.bank.alternatives[1].requestedF0Hz,compact.forecast.bank.alternatives[1].simulatedF0Hz,compact.forecast.bank.alternatives[0].simulatedF0Hz],['two_mass',180,171.5,null]);
 const ranked=compact.score.conditionalRanking.alternatives[1];assert.deepEqual([ranked.discrepancy,ranked.discrepancyExcludingPitch,ranked.heldoutRankExcludingPitch,ranked.requestedF0Hz,ranked.simulatedF0Hz],[2,1.5,1,180,171.5]);assert.equal(compact.score.conditionalRanking.conditionalRanking.rankingId,'rank-one');
