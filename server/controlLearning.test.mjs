...
function forecast(key,{hypotheses=2,matched=0,status='committed',baseline='model-one',binding='cue-one',at='2026-09-11T00:00:01+00:00'}={}){
 const ids=['reference','jaw-more-open'],learned=matched>=3,weights=learned?{reference:.2123456,'jaw-more-open':.7876544}:{reference:.5,'jaw-more-open':.5};
 ...
 return {artifact:{sha256:'f'.repeat(64),artifact:{compatibility_sha256:key,status:'available',alternatives:Object.keys(by).flatMap((sha,i)=>ids.map(id=>({anatomy_sha256:sha,hypothesis_id:'h'+i,control_id:id}))),
  conditional_predictions:Object.keys(by).map((sha,i)=>({anatomy_sha256:sha,indistinguishable_control_ids:i?[]:[['reference','jaw-more-open']]})),execution_support:{status:learned?'empirical':'uniform_insufficient_matches',anatomy_control_tradeoff:false,excluded:[],by_anatomy:by},empirical_residual_calibration:{by_anatomy:residual}}},
  status,committed_at:at,baseline_model_id:baseline,binding_id:binding};
}
const scored=(key,sha,status='scored')=>({operation:'score_control_pcm',binding_id:'cue-one',forecast_id:'t',status,result:{artifact:{compatibility_sha256:key,control_support:status==='unscorable'?{}:{[sha]:{reference:.1,'jaw-more-open':.9}}}}});
const binding=(wording,at='2026-09-11T00:00:00+00:00')=>({cue:{wording,wording_sha256:'a'.repeat(64),mode:'elicited'},context:{vowel:'a',pitch_hz:180},controls:[{control_id:'reference',JA:-3,f0_hz:180},{control_id:'jaw-more-open',JA:-4,f0_hz:180}],gain:4,declared_at:at});
...
test('context counts matched attempts per anatomy, keeps unsuccessful attempts visible and residuals separate',()=>{
 ...
 assert.deepEqual(first.weights,{reference:.2123,'jaw-more-open':.7877});assert.equal(first.supportStatus,'empirical');
 assert.deepEqual(first.residualCalibration,{status:'empirical',count:3,features:{pitchHz:{unit:'Hz',mean:1.2346,sd:null}}});
 assert.equal(row.latestForecast.current,true);assert.deepEqual(first.indistinguishableControlIds,[['reference','jaw-more-open']]);assert.deepEqual(second.indistinguishableControlIds,[]);
