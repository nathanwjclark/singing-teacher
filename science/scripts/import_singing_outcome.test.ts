...
with tempfile.TemporaryDirectory() as temp:
 ...
 with JobService(root/'jobs') as service:
  ...
  with Engine() as e:
   ...
   pcm=e.synthesize('a',{'JA':-2.},f0_hz=180.,duration_s=.25)*.1
  raw=pcm.astype('<f4').tobytes();(capture/'voice.raw').write_bytes(raw)
  rate=44100;count=len(pcm)
  ...
  config=dict(kind='singing_outcome_import_configuration',schema_version='0.1.0',command_id='outcome-import',participant_id='fixture-participant',session_id='session',design_id='design',experiment_id='a',observation_id='target',artifact_id='new-outcome-frame',pose='a',expected_version=state['version'],segment_index=0,frame_start_sample=4410,recording_kind='ordinary-singing',contains_external_excitation=False,evidence_kind='development-fixture',session_state_artifact=dict(path='state.json',sha256=sha(statebytes),byteCount=len(statebytes)),source_manifest_sha256=sha(mb))
  ...
  assert command['parameters']['pcm']==pcm[4410:8506].astype('float32').tolist()
...
`
 const output = execFileSync(process.env.OUTCOME_PYTHON ?? 'python3', ['-c', script], { encoding: 'utf8', timeout: 90000, env: { ...process.env, OUTCOME_NODE: process.execPath, OUTCOME_SCRIPT: resolve('science/scripts/import_singing_outcome.ts') } })
