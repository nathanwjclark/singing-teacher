...
Source inference remains explicitly disabled in this runtime. Existing frozen forecast descriptors, source assumptions, scores and model updates are unchanged. Astra may use the optional acoustic context to explain an already supported experiment; it must not add a source action absent from the actual frozen forecast.
