...
The `files` object contains exactly `tract0.obj`, `tract0.mtl`, `tract.svg`, `geometry.json`, `manifest.json` and `audio.wav`. These are the completed forward export's files. `audio.wav` is the native synthesized float32 waveform for the same forward request; the model teaching comparison (`science/scripts/app_teaching.py`) replays it through the frozen extractor before playback. The endpoint does not expose raw microphone/depth media or caller-selected paths. Existing bearer-token, host and origin restrictions apply unchanged.
...
Two actual HTTP tests pass: native forward-job export matches its verified files byte-for-byte (including a decodable 44.1 kHz float32 `audio.wav` of the requested duration), tampering fails, and unknown/cancelled/path-bearing requests are rejected.
