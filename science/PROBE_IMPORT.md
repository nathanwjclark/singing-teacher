...
The pull receipt closes that relabelling path for pulled archives. `server/nativePull.mjs` writes `native-pull-latest.json` as `native-pull-receipt-2`: the existing `name`, `bytes`, `sha256` and status fields, plus `acquisition` (`transport:"devicectl"`, `connection`, `container`, and `device` with `coreDeviceId`, `udid`, `productType`, `osVersion`) and `signature:{status:"not-provided"}`. `prepare_probe_capture.py` copies it to `usb-receipt.json` beside `original.zip`, and the app passes that path to this importer at import, at setup verification and again before a fit. The importer then checks `sha256` and `bytes` of `original.zip` against the receipt (refusing on mismatch) and classifies with `probeSource(manifest, acquisition)`:

| Pull receipt | Manifest says `software-fixture` | Manifest says `human-recording` or `physical-reference` |
|---|---|---|
| `devicectl` | human-recording (`attestation: devicectl-receipt`) | manifest rule (`devicectl-receipt`) |
| `repository-fixture` | software-fixture only with the fixture marker and no iPhone recorder fields; otherwise the import is refused (`contradicted-fixture-receipt`) | import refused (`contradicted-fixture-receipt`) |
| no transport (older receipt, or one this code does not know) | human-recording (`legacy-receipt`) | manifest rule (`legacy-receipt`) |
| no receipt (direct command-line import) | manifest rule (`none`) | manifest rule (`none`) |

"Manifest rule" is the field-based rule above. The receipt records `attestation` and `acquisition` (null without a receipt or for an older one), and a label overridden by the receipt gets its own reason. Only the repository's fixture writers produce `repository-fixture` receipts.

What this does not prevent: there is no signature yet, so anyone with shell access can edit `.local-data`, receipt included; files pushed into the development-signed app container with `devicectl device copy to` are pulled like the app's own; a human recording whose manifest is edited to say `physical-reference`, without iPhone recorder fields, keeps that label; AirDrop imports carry no pull receipt; and a synthetic sound played into the microphone is a real recording of a synthetic sound. A per-capture Secure Enclave signature from the iPhone app, verified at pull time, is the planned next stage for the container-injection and after-the-fact editing gaps. `scripts/import-acoustic-probe.ts` still classifies from the manifest alone, so its review record (`provenance` in `probe-measurement.json`, including the copy under `b-import/`) can say `software-fixture` where this importer's receipt says `human-recording`; eligibility and the app's fit gate use this importer's classification.
