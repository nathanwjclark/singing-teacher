...
"Manifest rule" is the field-based rule above. The receipt records `attestation` and `acquisition` (null without a receipt or for an older one), and a label overridden by the receipt gets its own reason. B's review record under `b-import/` is classified with the same acquisition record, and the app's review-only import passes the receipt to `scripts/import-acoustic-probe.ts` (optional third argument), so every `probe-measurement.json` agrees with this receipt about the source kind. Only the repository's fixture writers produce `repository-fixture` receipts.
