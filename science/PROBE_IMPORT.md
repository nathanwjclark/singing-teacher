...
The pull receipt closes that relabelling path for pulled archives. `server/nativePull.mjs` writes `native-pull-latest.json` as `native-pull-receipt-2`: the existing `name`, `bytes`, `sha256` and status fields, plus `acquisition` (`transport:"devicectl"`, `connection`, `container`, and `device` with `coreDeviceId`, `udid`, `productType`, `osVersion`) and `signature:{status:"not-provided"}`. `prepare_probe_capture.py` copies it to `usb-receipt.json` beside `original.zip`, and the app passes that path to this importer at import, at setup verification and again before a fit. The import summary records the SHA-256 of the receipt bytes it wrote (`receiptSha256`), and this importer records the SHA-256 of the receipt bytes it read (`receipt_sha256`). The fit always re-imports with the receipt and refuses if the receipt is missing, the summary has no `receiptSha256`, or the two hashes differ, so a pulled capture cannot drop to the no-receipt rule between import and fit. A probe imported from the command line without a receipt therefore cannot be fitted in the app; the app only fits imports made from a USB pull. The importer then checks `sha256` and `bytes` of `original.zip` against the receipt (refusing on mismatch) and classifies with `probeSource(manifest, acquisition)`:
