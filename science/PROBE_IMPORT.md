...
"Manifest rule" is the field-based rule above. A pulled archive labelled `physical-reference` is a human recording because the only probe writer in the app container, `apps/ios/SingingDepth/SingingDepth/AcousticProbe.swift`, always writes `provenance: "human-recording"` with its recorder fields; any other label on a pulled archive came from an injected or edited file. A `physical-reference` capture imported from the command line without a receipt keeps its label, and whether that stays allowed is an owner decision. A `provenance` value outside the three labels (a different spelling, a missing value, an array) is a human recording. The receipt records `attestation` and `acquisition` (null without a receipt or for an older one), and a label overridden by the receipt gets its own reason. B's review record under `b-import/` is classified with the same acquisition record, and the app's review-only import passes the receipt to `scripts/import-acoustic-probe.ts` (optional third argument), so every `probe-measurement.json` agrees with this receipt about the source kind. Only the repository's fixture writers produce `repository-fixture` receipts.
