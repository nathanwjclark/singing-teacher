...
def _worker(root, job_id, parent_pid, timeout_s):
    ...
    try:
        ...
        elif request['operation'] == 'evaluate_probe':
            from .prediction import Artifact
            from .probe_evaluation import evaluate_probe
            artifact = evaluate_probe(Artifact(params['forecast_json'].encode()), **{k:v for k,v in params.items() if k != 'forecast_json'})
            artifact.write(destination / 'evaluation.json')
            result = artifact.data
...
class JobService:
    ...
    def submit(self, request, *, idempotency_key):
        ...
        allowed = {
            ...
            'design_pcm': {'snapshot_json', 'expected_digest', 'design_id', 'target_observation_id', 'generated_at', 'experiments', 'feature_scales', 'minimum_separation', 'max_synthesis_calls', 'retention_margin', 'maximum_discrepancy', 'profile', 'objective'},
            ...
            'fit_pcm': {'observations', 'candidates', 'max_synthesis_calls', 'objective'},
