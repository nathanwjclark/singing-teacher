...
class JobService:
    ...
    def submit(self, request, *, idempotency_key):
        ...
        if request.get('operation') not in {'forward', 'fit_transfer', 'fit_joint', 'predict', 'fit_dynamic', 'fit_control', 'control_predict', 'condition_prediction', 'fit_frozen_control', 'rank_interventions', 'fit_pcm', 'search_pcm', 'design_pcm', 'update_pcm'} or not isinstance(request.get('parameters'), dict):
        ...
        if request['operation'] in {'predict', 'control_predict', 'condition_prediction', 'fit_frozen_control', 'rank_interventions', 'design_pcm', 'update_pcm'} and 'model_id' in request:
