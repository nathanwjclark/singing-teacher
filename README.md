...
The Tongue layer is an original, simplified 3D surface attached to the jaw. Its exposed tip follows an experimental local color-segmentation estimate in a detailed mouth crop, including an exposed tip connected to the mouth below the inner lip. A pink outline and crosshair mark the detected region, and a dashed search box plus a camera status label make detection state visible. Two consecutive detections are required, and tracking clears when the region is lost. The model returns to its resting reference when the tongue is not visible. The Tongue demo uses synthetic motion.
