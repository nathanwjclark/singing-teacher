...
| P1 | Reused request IDs could return results for a changed vowel/model declaration, and the UI could repeatedly retry the rejected old ID after a baseline change. | The server rejects mismatched declarations. UI correction `375faad` fetches fresh status before submission and keys retries by capture, vowel and current baseline model. Uncertain retries retain their ID; changed baselines receive a new one. Independently verified all five motion-client tests. |
...
- **Reviewer-run:** `src/components/motion/motionClient.test.ts`: **5 passed**, including fresh-baseline retry identity and preservation of unavailable candidate scores rather than invented zeros. Component inspection confirms the fresh status request precedes request-ID selection and POST.
...
- **Independent browser QA owner supplied:** final fresh-bundle rerun after `375faad`: native real-server case **1 passed in 26.0 seconds**, displaying POST acceptance, busy state, the actual 72-call result, model/subset and reachable download with unchanged baseline. Separate real-server no-baseline and missing-decoder cases each passed, in **4.4 seconds** and **4.3 seconds**. Each mode intentionally selected its applicable case; these are three executed cases, not an all-tests run with every skipped case counted as a pass. Browser test commit: `1761241`.

All concrete findings in this review are resolved and verified at the described boundaries. No outstanding software correctness blocker remains from this review. Final real-server browser verification after the UI correction passed as recorded above. Its execution was supplied by the separate browser QA owner, not rerun by this reviewer.
