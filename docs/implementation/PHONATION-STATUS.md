...
The normal five-anatomy profile retains two matched source-shape alternatives per anatomy: 30 fit calls and 30 bank calls. Hard bounds are 16 source candidates and 48 bank alternatives; deadlines remain bounded. See [source bank budgets](../../science/SOURCE_BANK_BUDGET.md). Changed source/extractor policy invalidates cached optional results and allows reanalysis; old receipts remain history. Native two-round and browser verification distinguish software execution from physical closure validation.
