...
| Native two-mass source family | Opt-in with `PHONATION_SOURCE_MODEL=two_mass` through the same fit, frozen bank and scoring; per-family capability and provenance; certified speaker restored after every switch | Geometric glottis stays the default. The frozen equal-budget comparison is inconclusive (geometric won more cases, mostly on geometric-generated audio); two-mass simulated pitch can miss the requested F0 by tens of Hz, so every row shows both and a pitch-excluded discrepancy |
