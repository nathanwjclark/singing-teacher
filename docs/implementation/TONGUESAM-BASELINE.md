...
Verification: `npm run build`, `npm run lint`, and nine focused Node tests passed. `TONGUE_PUBLIC_FIXTURE=<path to pinned data/test_in/1.jpg> npx playwright test --config tests/tongue-runtime.config.ts` passed the real Chrome WASM worker → detected region → calibration → freeze → independent surface/hidden-tip annotations → export flow, plus gray-frame abstention and missing-model capture. It recorded a region score of 0.9631 and gray-frame inference of 375 ms on this Mac. The test verifies the input SHA-256 and downloads the pinned public example when the environment variable is omitted. The image is not committed. Mobile 390-pixel layout and zero page exceptions passed; this is public-example execution evidence, not held-out accuracy or device performance.
