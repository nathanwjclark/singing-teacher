...
4. Repeat each arm without its mnemonic, transfer to the declared phrase **on the same note**, then revisit after the frozen delay in a new page session for retention. Assistance and replay are hidden in these stages; selected stage persists across reload. Without a declared melody this checks a narrowly defined single-note pitch task. A protocol can instead freeze a short 2–8 note melody for prompted practice and phrase transfer; see [../evaluation/MELODIC_TRANSFER.md](../evaluation/MELODIC_TRANSFER.md) for its scoring and limits.
