...
Sensations are explicitly subjective. They never overwrite anatomy. Movement agreement is `null`, not inferred from pitch. Local UI records use `singing-teacher/learning/1`; they are a browser package distinct from the shared motion/control interchange records. Fitting and control-forecast adaptation remains a producer boundary.

## Acceptance evidence and remaining gates

- Build/type check and lint.
- Three focused tests cover review requirement, protocol tampering, missing/unvoiced/context-invalid evidence, failed-attempt denominator, cue-free assistance exclusion and delayed separate-session retention.
