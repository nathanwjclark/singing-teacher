...
Region reporting compares the detector box with the bounding box of each manually labeled visible surface. In a lab export, `regionPrediction: null` is a detector abstention and counts as a miss; a frame without `regionPrediction` had no current detector result (loading, pending or expired) and is excluded from box IoU, reported as `reviewedFramesWithoutObservation`. These are box IoU measurements, never segmentation IoU, and absent-surface false detections are separate. Original recorded acquisition timestamps remain in report rows because asynchronous observations may lag the image.
...
`scripts/tongue-benchmark.test.mjs` runs in `npm test`. It covers independent surface/tip labels and misses, missing capability, recorded region boxes, frames without a current region result, and rejection of mismatched/duplicate/invalid predictions. `tests/tongue-runtime.spec.ts` verifies freeze → outline → hidden tip → export in Chrome, preserving both annotations, region boxes, abstentions and session ID without page errors. This is software verification, not detector accuracy.
