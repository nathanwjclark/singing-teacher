...
One internal scorer edge case was sent to the source owner: removing an extractor file after forecast commit raises a file error rather than returning unsupported capability. Its changed-version check must also catch missing files; this does not affect core app operation while source inference remains disconnected and disabled. Final closure of that finding requires its targeted regression.
