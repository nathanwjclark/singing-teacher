...
The comparison removes an unbounded constant log-power difference **separately at each resolution** to isolate shape. It then fits one shared spectral tilt within ±6 dB/octave. Raw shape error, adjusted shape error and the fitted tilt are all retained. Overall RMS level stays separate: a scalar gain adjustment is allowed within ±24 dB, and any level difference beyond that is retained and penalized. In plain terms, a loudness difference of up to 24 dB between prediction and recording adds nothing to the score (a frame scaled by 0, −6 or −23 dB scores 0 on level; −30 dB scores 4.0). This is deliberate invariance to microphone gain, and it also discards real evidence: native level can differ by tens of dB between anatomies (about 46 dB for vowel e between the two revision-2 anatomies). The app's objective label says this directly. These nuisance parameters stand for unresolved source and recording differences. They do not measure glottal closure or calibrate a room.
...
A new design records `objective`, `objective_policy` (for the spectral objective: its configuration, the SHA-256 of `pcm_spectral.py` and the NumPy version) and `scorer_implementation_pin`: SHA-256 hashes of `pcm_design.py`, `pcm_inverse.py`, `pcm_spectral.py`, `prediction.py`, `engine.py` and `science/scripts/extract_pcm.ts`, plus NumPy and SciPy versions. Each extraction runs a new Node process that reports the bridge file's hash, and a reply that differs from the hash taken at import is rejected. The TypeScript extractor and contracts are pinned separately by that subprocess and checked on update. A spectral design must declare pitch and periodicity scales equal to the policy's 20 Hz and 0.1, because the score uses the policy values. Fit and search results record `objective_policy`.
