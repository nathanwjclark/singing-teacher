...
3. **Measured probe calibration:** derive calibration arrays from a reference-microphone sweep. The USB pull now records the device, connection and archive hash, and the importer checks the archive against that receipt, so a pulled recording cannot be relabelled as a fixture; the receipt is still unsigned. Next: the iPhone app signs each capture with a Secure Enclave key and the pull verifies it (needs Xcode).
...
5. **Session ledger size (resolved on `claude/ledger-v2`):** full-state events made the replay grow quadratically (the four-forecast app loop reached 23.9 MB of the 25.2 MB export bound). New events carry only the digest of a content-addressed state tree whose nodes are stored once per session ([ledger format](../../science/SESSION.md)); v1 events stay untouched. The app loop now stores 0.92 MB and replays 1.98 MB, adding about 0.29 MB per Astra round (rising about 5 KB per round, so the export bound is about 54 rounds away). Returning to older code after the first new write needs a pre-upgrade database backup. Still open: the read-only `GET /sessions/:id/ledger` route does not run the control-digest check (`session_control.verify_ledger`) that `state` and `replay` run.
