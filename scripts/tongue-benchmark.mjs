...
export function benchmark(review,predictions,reviewSha256){
 ...
 const errors=[],ious=[],rows=[];let visible=0,tipFound=0,hidden=0,falseTip=0,surfaces=0,surfaceFound=0,absent=0,falseSurface=0;
 for(const [index,s] of review.samples.entries()){
  ...
  const predicted=predictions?byIndex.get(index):{tip:s.prediction??null};
  ...
  const row={index,imageSha256,tipReviewed:s.label!==undefined,surfaceReviewed:s.surface!==undefined,tipDetected:!!predicted?.tip,surfaceDetected:!!predicted?.surface};
  ...
  if(s.surface){surfaces++;surfaceFound+=Number(!!predicted?.surface);const iou=overlap(s.surface,predicted?.surface);row.surfaceIoU=iou;if(iou!==null)ious.push(iou);}
  else if(s.surface===null){absent++;falseSurface+=Number(!!predicted?.surface);}
  rows.push(row);
 }
 return {schema:'tongue-benchmark/v1',reviewSha256,sessionId:review.sessionId??null,modelId:predictions?.modelId??'recorded-live-tip-predictions',
