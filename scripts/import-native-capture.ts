...
export async function importNativeCapture(directory:string,participantId:string,sessionId:string){
 ...
 if(manifest.schema_version!=='singing-native-rgbd-1.0.0'||manifest.capture_mode!=='one-held-pose')throw Error('Unsupported native capture schema/mode')
 const device=row(manifest.device,'device');if(!text(device.device_type,'device type').includes('TrueDepth')||device.position!=='front'||device.output_mirrored!==false)throw Error('Expected native unmirrored front TrueDepth source metadata')
