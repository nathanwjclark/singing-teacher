...
import { PROBE_VERSION,probeSource,validateProbeMeasurement } from '../src/contracts/probes.ts'
...
export async function importAcousticProbe(directory:string,output:string){
 const root=await realpath(directory)
 async function read(name:string){if(typeof name!=='string'||name!==basename(name)||!/^[\w][\w.-]*$/.test(name))throw Error('Unsafe artifact path');const file=await open(resolve(root,name),constants.O_RDONLY|constants.O_NOFOLLOW);try{const stat=await file.stat();if(!stat.isFile()||stat.size>128*1024*1024)throw Error('Invalid artifact size/type');const bytes=await file.readFile();if(bytes.length!==stat.size)throw Error('File changed during read');return bytes}finally{await file.close()}}
 ...
 // Review records carry the source kind the manifest's fields support, not its bare label.
 const source=probeSource(a).kind
