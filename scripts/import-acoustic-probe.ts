...
async function readFrom(root:string,name:string,limit:number){if(typeof name!=='string'||name!==basename(name)||!/^[\w][\w.-]*$/.test(name))throw Error('Unsafe artifact path');const file=await open(resolve(root,name),constants.O_RDONLY|constants.O_NOFOLLOW);try{const stat=await file.stat();if(!stat.isFile()||stat.size>limit)throw Error('Invalid artifact size/type');const bytes=await file.readFile();if(bytes.length!==stat.size)throw Error('File changed during read');return bytes}finally{await file.close()}}
/** The acquisition record of the app's `usb-receipt.json` (null for a receipt that has none), after checking the
 * sha256 and byte count of the `original.zip` beside it. Pass the result to probeSource. */
...
export async function importAcousticProbe(directory:string,output:string,acquisition?:unknown){
 ...
 const read=(name:string)=>readFrom(root,name,128*1024*1024)
 ...
 const source=probeSource(a,acquisition).kind
