...
|---|---|---|---|
| `src/contracts/probes.ts` | `2c9001d74f1e` | `5cde7a42c43f` | Changed on `claude/wave3-probe`, the base branch, and on `claude/pull-provenance`, which classifies captures with their pull receipt. Hashed because every contract file is in the extractor signature; the phonation descriptor code (`src/phonation/measure.ts`, `src/lib/audio.ts`, `src/phonation/types.ts`) is unchanged. |
| `src/contracts/contracts.test.ts` | `7ff8e1cd57f4` | `a69de110d340` | Test file, changed on `claude/pull-provenance` to cover receipt-aware probe classification. Hashed only because the extractor signature includes every contract file; the phonation descriptor code does not import it, so the descriptors and this run's results are unaffected. |
