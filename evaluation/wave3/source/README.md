...
| `science/src/singing_physics/pcm_inverse.py` | `2e0a3fb5a1ed` | `e6ded2acae9f` | Changed on `claude/wave3-spectral`, which follows this branch in the PR stack. `fit_pcm` takes an explicit scoring objective (coarse, the objective this comparison used, stays the default), candidates that differ only in gain share one synthesized waveform with identical scores, and the extraction bridge reports its own hash. None of these changes the coarse discrepancy of a given frame. |
