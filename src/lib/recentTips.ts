...
export type RecentTip = Tip & { triggeredAt: number; lastSeenAt: number; active: boolean };
export const RECENT_WINDOW_MS = 45_000;
...
export function updateRecentTips(previous: RecentTip[], current: Tip[], now: number): RecentTip[] {
  const rows = new Map(previous.filter(row => now - row.lastSeenAt < RECENT_WINDOW_MS).map(row => [row.id, { ...row, active: false }]));
