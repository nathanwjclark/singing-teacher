...
export function ProbeSetupPanel({ status, busy, onSaved }: { status?: ProbeSetupStatus; busy: boolean; onSaved: () => void }) {
  ...
  // Values copied from the last setup (same placement id only) stay marked and need their own confirmation.
  const [prefill, setPrefill] = useState<{ from: string; positions: boolean; controls: string[] } | null>(null), [skipped, setSkipped] = useState('')
  const [checked, setChecked] = useState({ positions: false, controls: false })
  const capture = status?.capture, last = status?.setup
  async function loadPackage(file?: File) {
    setError(''); setNotice(''); setDeclared(false); setCalibration(null); setPackageFile(null); setFiles([]); setPrefill(null); setSkipped(''); setChecked({ positions: false, controls: false })
    if (!file) return
    try {
      ...
      // Package values first. The last setup fills gaps only when it used the same placement id, so one rig's
      // positions are never saved under another rig's placement.
      const packagePlacement = value.placement?.placement_id ?? value.calibration.placement_id
      const from = last && last.placement.placement_id === packagePlacement ? last : null
      const fixed = (key: string) => { const bound = value.nuisance_prior.bounds[key]; return bound?.length === 2 && bound[0] === bound[1] ? String(bound[0]) : null }
      const placement = value.placement ?? from?.placement
      setPlacementId(packagePlacement ?? capture?.placementId ?? '')
      ...
      setProfile(Object.fromEntries(controls.map(key => [key, fixed(key) ?? (from ? String(from.profile[key]) : '')])))
      setPrefill(from ? { from: from.setupId, positions: !value.placement, controls: controls.filter(key => fixed(key) === null) } : null)
      if (last && !from) setSkipped(`The last setup ${last.setupId} used placement ${last.placement.placement_id}; this package declares ${packagePlacement}, so none of its values were filled in.`)
  ...
  async function save() {
    ...
    try {
      ...
      if (!confirmed) throw new Error('Confirm the values filled in from the last setup.')
  ...
  const confirmed = (!prefill?.positions || checked.positions) && (!prefill?.controls.length || checked.controls)
  const fromLast = (show: boolean | undefined) => show && prefill && <small className="probe-prefilled">From setup {prefill.from}</small>
  return <section className="probe-setup" aria-label="Probe calibration setup" aria-busy={saving}>
    <h4>Calibration setup</h4>
    <p>Each probe capture needs its own setup: a setup binds that capture's manifest hash, held-quiet pose, placement and route. When the setup is saved, the app analyzes the probe again with it.{last && ' If the package declares the same placement as your last setup, its positions and controls are filled in and marked; confirm them before saving.'}</p>
    ...
    {!human && <fieldset disabled={!capture || busy || saving}>
      ...
      {calibration && <>
        ...
        <div className="probe-setup-grid">
          ...
          <label>Coordinate frame<input value={frame} onChange={e => { setFrame(e.target.value); setDeclared(false) }}/>{fromLast(prefill?.positions)}</label>
        ...
        {positions.map(key => <fieldset key={key}><legend>{positionLabels[key]}{prefill?.positions && ` · from setup ${prefill.from}`}</legend><div className="probe-coordinate-grid">{['X', 'Y', 'Z'].map((axis, i) => <label key={axis}>{axis}<input aria-label={`${positionLabels[key]} ${axis}`} type="number" step="any" value={coordinates[key][i]} onChange={e => { setCoordinates(old => ({ ...old, [key]: old[key].map((v, n) => n === i ? e.target.value : v) })); setDeclared(false) }}/></label>)}</div></fieldset>)}
        {prefill?.positions && <label className="probe-declaration"><input type="checkbox" checked={checked.positions} onChange={e => setChecked(old => ({ ...old, positions: e.target.checked }))}/>The coordinate frame and positions from setup {prefill.from} still match this rig.</label>}
        ...
        <div className="probe-setup-grid">{controls.map(key => <label key={key}>{labels[key]}<input type="number" step="any" value={profile[key]} onChange={e => { setProfile(old => ({ ...old, [key]: e.target.value })); setDeclared(false) }}/>{fromLast(prefill?.controls.includes(key))}{key !== 'JA' && <small>Prior: {calibration.nuisance_prior.bounds[key]?.join(' to ')}</small>}</label>)}</div>
        {!!prefill?.controls.length && <label className="probe-declaration"><input type="checkbox" checked={checked.controls} onChange={e => setChecked(old => ({ ...old, controls: e.target.checked }))}/>The controls from setup {prefill.from} still apply to this capture.</label>}
        ...
        <button disabled={!declared || !confirmed || !files.length} onClick={() => void save()}>Verify and save calibration setup</button>
      </>}
    </fieldset>}
