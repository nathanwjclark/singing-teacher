...
export function SideAnatomyPanel({ motion }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (status.current) status.current.textContent = state.demo ? 'DEMO MOTION' : state.frame ? 'SHARED CAMERA MOTION' : 'REFERENCE · WAITING FOR CAMERA';
  ...
  return <div className="side-anatomy-panel">
    ...
    <footer><span ref={status}>REFERENCE · WAITING FOR CAMERA</span><p>Reference internal shape · motion shared with front model. Internal shape is not measured.</p></footer>
  </div>;
}
