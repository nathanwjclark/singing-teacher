...
export function ScientificSideView(){
 ...
 useEffect(()=>{if(!result)return;let active=true,url='';void verifiedScienceAsset(result,'tract.svg').then(bytes=>{if(active){url=URL.createObjectURL(new Blob([bytes],{type:'image/svg+xml'}));setImage(url)}}).catch(e=>{if(active)setError(String(e))});return()=>{active=false;if(url)URL.revokeObjectURL(url)}},[result?.runId]);
