...
export function ScientificModelPanel({onPreview}:{onPreview:()=>void}){
 ...
 const [voiceOnly,setVoiceOnly]=useState(false),[outcomeConfirmed,setOutcomeConfirmed]=useState(false),[objective,setObjective]=useState('canonical-coarse-v1');
 ...
 async function run(purpose:'calibration'|'outcome',resume=false){
  ...
  try{
   ...
   const launched=await (purpose==='calibration'?scienceAction('run',{objective}):scienceAction('outcome'));
 ...
 <label><input type="checkbox" checked={voiceOnly} onChange={event=>setVoiceOnly(event.target.checked)} disabled={busy}/> This latest recording contains only my sustained a (ah) vowel, with no played probe or external excitation.</label>
 <label>Scoring objective <select value={objective} onChange={event=>setObjective(event.target.value)} disabled={busy}>{Object.entries(OBJECTIVES).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
 <span>Both objectives use the same native synthesis budget. Neither is a calibrated likelihood; a better fit does not establish correct anatomy.</span></div>
