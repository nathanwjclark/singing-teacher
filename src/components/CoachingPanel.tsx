...
export function CoachingPanel({ tips, demo, tracking, selectedTipId, now, onSelectTip }: Props) {
  ...
  return <section className="coaching-panel recent-coaching" aria-label="Recent coaching adjustments">
    ...
    {selected && <div className="selected-cue-detail" id="selected-cue-detail">{!selected.active && <span>{selected.resolved ? 'ADJUSTMENT OBSERVED' : 'RECENT CUE · NOT CURRENTLY DETECTED'}</span>}<p>{selected.detail}</p>{selected.muscles?.length ? <span>REFERENCE · {selected.muscles.join(' / ')}</span> : null}</div>}
  </section>;
}
