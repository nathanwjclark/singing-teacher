...
test('all mechanisms render with keyboard scrubbing, paused animation and reduced-motion static views',async()=>{
 ...
 try{browser=await chromium.launch({channel:'chrome',headless:true});const page=await browser.newPage({viewport:{width:900,height:900}});const errors=[];page.on('pageerror',e=>errors.push(e.message));const base=`http://127.0.0.1:${server.httpServer.address().port}/teaching-preview`;
