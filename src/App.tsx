...
function App() {
  ...
  const shownFrame = demo ? demoFrame : active && (status === 'tracking' || status === 'no-face') ? frame : null
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {help && <aside className="help-box"><strong>Your practice, in three views.</strong> Allow camera access, frame your head and shoulders, and try a comfortable sustained vowel. The movement guide follows visible landmarks. Tips are experimental visual prompts, not an assessment of your voice or internal anatomy. Video stays in your browser; no microphone is used. <button onClick={() => setHelp(false)}>Got it</button></aside>}
        ...
        <div className="studio-grid">
          <section className="studio-column"><div className="column-title"><span className="column-number">01</span><h2>Your view</h2><Camera size={16}/></div><div className="panel-body camera-wrap"><CameraPanel active={active} onFrame={setFrame} onStatus={onStatus}/>{demo && <div className="demo-cover"><div className="demo-avatar"><MicVocal size={48}/></div><span className="eyebrow">SAMPLE SESSION</span><h3>A little room to open up.</h3><p>Example measurements show how movement becomes a coaching cue.</p><span className="demo-pill">Demo · camera is off</span></div>}</div><div className="panel-footer"><span className="tiny-dot"/>{demo ? 'Example data · not live analysis' : 'Frame your head and shoulders'}</div></section>
          <section className="studio-column"><div className="column-title"><span className="column-number">02</span><h2>Movement map</h2><Activity size={16}/></div><div className="panel-body"><AnatomyPanel frame={shownFrame} activeRegion={tips[0]?.region} demo={demo}/></div><div className="panel-footer"><Eye size={13}/> Visible movement, illustrated</div></section>
          <section className="studio-column coach-column"><div className="column-title"><span className="column-number">03</span><h2>Your next adjustment</h2><span className="live-tag">{demo ? 'DEMO' : active && status === 'tracking' ? 'LIVE' : 'COACH'}</span></div><div className="panel-body"><CoachingPanel tips={tips} demo={demo} tracking={!!shownFrame?.face.length}/></div><div className="panel-footer">One small change at a time.</div></section>
        </div>
