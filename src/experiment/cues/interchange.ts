...
import { evaluateLearning, isMelodic } from '../../evaluation/learning/index.ts'
import { MELODY_HOP_MS, MELODY_STABLE_MS, MELODY_TRAILING_SILENCE_MS } from '../../evaluation/learning/melody.ts'
...
export function learningInstruction(protocol: LearningProtocol, arm: LearningArm, phase: LearningAttempt['phase']) {
  ...
  if (phase === 'transfer' && protocol.melody) return `Sing the frozen short melody from memory using the declared words or vowel, without consulting your cue or mnemonic. Begin when ready, then leave at least ${MELODY_TRAILING_SILENCE_MS} ms of silence before stopping.`
...
export async function exportLearningInterchange(protocols: LearningProtocol[], attempts: LearningAttempt[]) {
  ...
  for (const protocol of protocols) {
    ...
    const context = { declaredContext: protocol.context, phrase: protocol.phrase, targetHz: String(protocol.targetHz), toleranceCents: String(protocol.toleranceCents), protocolDigest: protocol.digest, acousticErrorUnit: 'cents', scoring: protocol.scoring, ...(protocol.melody ? { melodicTarget: JSON.stringify(protocol.melody), melodicTiming: `anchored at the first ${MELODY_STABLE_MS} ms run within tolerance of note 1; note changes located in the pitch track; phrase ends at the first unvoiced run longer than ${protocol.melody.maximumBreathMs} ms; fixed declared tempo; no time warping; ${MELODY_HOP_MS} ms sample hop`, pitchReference: 'absolute declared frequencies; transposition is reported, not forgiven', articulation: 'unmeasured' } : {}), reviewEvidence: protocol.cue.review?.evidence || 'not supplied', reviewerRole: protocol.cue.review?.role || 'not supplied', physiologicalAgreement: 'not measured', ...(lineage ? { sourceSessionId:lineage.sessionId,sourceRunId:lineage.runId,sourceModelId:lineage.modelId,sourceDesignId:lineage.designId,sourceDecisionId:lineage.decisionId,sourceAttemptId:lineage.attemptId,sourceMemoryId:lineage.memoryId,sourceRecordedAt:lineage.recordedAt,cueOrigin:'historical Astra decision; source IDs are not current learning predictions' } : {}) }
    ...
    group.forEach((attempt, index) => {
      ...
      const attemptContext = { ...(score.melody ? { melodicScore: JSON.stringify(score.melody) } : {}), ...context, comparisonArm: attempt.arm, learningStage: attempt.phase, sessionId: attempt.sessionId, contextConfirmed: String(attempt.contextConfirmed), mnemonicAssistance: attempt.phase === 'prompted' ? 'available' : 'hidden', modelStatus: 'not connected', predictionStatus: 'not connected' }
      records.push({ ...base(attempt.id, createdAt, 'human-observation', evidenceIds, evidenceHashes), kind: 'cue-attempt', cueId: cueId(attempt.arm), cueVersion: String(protocol.cue.version), deliveredWording: attempt.phase === 'prompted' ? attempt.deliveredWording : learningInstruction(protocol, attempt.arm, attempt.phase), goal: protocol.melody && isMelodic(attempt.phase) ? `Reproduce the frozen ${protocol.melody.notes.length}-note melody within ${protocol.toleranceCents} cents and ${protocol.melody.rhythmToleranceMs} ms` : `Reproduce ${protocol.targetHz} Hz within ${protocol.toleranceCents} cents in the frozen task`, modelId: null, predictionId: null, context: attemptContext, startedAt: attempt.startedAt, endedAt: attempt.endedAt || null, repetition: index + 1, captureIds: attempt.recordingId ? [attempt.recordingId] : [], executionDeviations: [attempt.deviations, attempt.failureReason].filter((s): s is string => !!s?.trim()), outcome: attempt.outcome === 'failed' ? 'unsuccessful' : attempt.outcome })
