...
export async function exportLearningInterchange(protocols: LearningProtocol[], attempts: LearningAttempt[]) {
  ...
  for (const protocol of protocols) {
    ...
    const base = (id: string, createdAt: string, kind: LearningBase['provenance']['kind'], sourceIds: string[], sourceHashes: string[] = []): LearningBase => ({ schemaVersion: LEARNING_VERSION, id, createdAt, provenance: { kind, producer: 'singing-teacher/learning', producerVersion: '1.0.0', sourceIds, sourceHashes } })
