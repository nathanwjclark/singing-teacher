...
// Descriptor checks must not depend on machine load; the live 200 ms deadline is
// covered by the timeout tests in runtime.test.ts.
const measure:typeof measurePhonation=(pcm,rate,meta,options={})=>measurePhonation(pcm,rate,meta,{deadlineMs:2000,...options});
...
test('same extractor source invariance, finite acoustic slope and exact frame hash',async()=>{
 const pcm=wave(),r=await measure(pcm,48000,metadata),synthetic=await measure(pcm,48000,{...metadata,sourceKind:'engine-generated'});
 ...
 const steeper=await measure(wave(180,2),48000,metadata);assert.ok(steeper.descriptors.harmonicSpectralSlopeDbOctave.value! < r.descriptors.harmonicSpectralSlopeDbOctave.value!-4);
...
test('silence, clipping, noise, high pitch and nonfinite have explicit unavailable reasons',async()=>{
 ...
 for(const pcm of [new Float32Array(4096),new Float32Array(4096).fill(1),noise,wave(1000)]){
  const r=await measure(pcm,48000,metadata);assert.equal(r.capabilities.measurement.status,'insufficient-quality');assert.ok(Object.values(r.descriptors).every(d=>d.value===null&&d.reason));
 }
 const invalid=wave();invalid[0]=NaN;assert.equal((await measure(invalid,48000,metadata)).capabilities.measurement.status,'failed');
 assert.equal((await measure(wave(),22050,metadata)).capabilities.measurement.status,'unsupported');
});
test('disabled cancellation and insufficient harmonic support are distinct',async()=>{
 assert.equal((await measure(wave(),48000,metadata,{enabled:false})).capabilities.measurement.status,'disabled');
 const abort=new AbortController();abort.abort();assert.equal((await measure(wave(),48000,metadata,{signal:abort.signal})).capabilities.measurement.status,'timed-out');
 const sine=Float32Array.from({length:4096},(_,i)=>.1*Math.sin(2*Math.PI*180*i/48000));
 const r=await measure(sine,48000,metadata);assert.equal(r.capabilities.measurement.status,'available');assert.equal(r.descriptors.harmonicSpectralSlopeDbOctave.value,null);
});
test('asynchronous frame hash and analysis share one immutable buffer snapshot',async()=>{
 const pcm=wave(),expected=await measure(pcm,48000,metadata);
 const pending=measure(pcm,48000,metadata);pcm.fill(0);const actual=await pending;
...
test('strongly time-varying nonstationary signal is not a stable phonation reference',async()=>{
 ...
 const result=await measure(chirp,48000,metadata);assert.equal(result.capabilities.measurement.status,'insufficient-quality');
});

test('unknown absolute evidence time stays null and invalid timestamps reject',async()=>{
 const result=await measure(wave(),48000,{...metadata,evidenceAt:null});
 ...
 await assert.rejects(measure(wave(),48000,{...metadata,evidenceAt:'invalid-date'}),/evidence time/);
});
