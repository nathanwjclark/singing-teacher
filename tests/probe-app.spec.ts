...
test('probe errors remain visible and browser JSON review never submits a scientific fit',async({page})=>{
 ...
 await mapping.getByLabel('Import local probe-measurement.json').setInputFiles({name:'probe-measurement.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(review))});
 ...
 await mapping.getByLabel('Import local probe-measurement.json').setInputFiles({name:'bad.json',mimeType:'application/json',buffer:Buffer.from('{"includedInFit":true}')});await expect(mapping).toContainText('Import failed:');expect(submissions).toBe(0);
});
