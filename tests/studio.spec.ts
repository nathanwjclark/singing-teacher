...
test('camera denial remains actionable and supports retry', async ({ page }) => {
  ...
  await page.goto('/');
    await expect(page.locator('.session-message')).toContainText('Camera permission was denied');
...
test('ending a session releases a camera permission request that resolves late', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    state.stopped = false;
    navigator.mediaDevices.getUserMedia = (constraints) => constraints?.audio ? Promise.reject(new DOMException('Denied', 'NotAllowedError')) : new Promise(resolve => {
