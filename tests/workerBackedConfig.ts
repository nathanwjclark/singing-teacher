...
export function workerBackedConfig({spec,prefix,port,sciencePort,data,timeout,prepare='tests/fixtures/prepare_voice_browser.py',appData=''}:{spec:string;prefix:string;port:number;sciencePort:number;data:string;timeout:number;prepare?:string;appData?:string}){
  ...
  return defineConfig({
    ...
    webServer:{
      ...
      env:{PORT:String(appPort),SCIENCE_PORT:String(workerPort),LOCAL_DATA_DIR:appData?`${data}/${appData}`:data,SCIENCE_DATA_DIR:`${data}/science-jobs`,
