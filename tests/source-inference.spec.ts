...
const fit={status:'succeeded',result:{status:'completed',actual_synthesis_calls:12,source_model_version:'software-test',joint:{best:{score:1.2},candidates:[{candidate_id:'h1',source_model:'geometric',status:'scored',score:1.2,score_excluding_pitch:1.1,predictions:[{controls:{PS:800,F0:180,JA:-3,gain:1},requested_f0_hz:180,simulated_f0_hz:180.2}],anatomy:{palate_depth:3.6}},{candidate_id:'h2',source_model:'two_mass',status:'scored',score:1.4,score_excluding_pitch:0.9,predictions:[{controls:{source_model:'two_mass',XB:.005,XT:.005,EAA:0,DF:1,F0:180,JA:-3,gain:1},requested_f0_hz:180,simulated_f0_hz:171.5}],anatomy:{palate_depth:3.8}}]},fixed_source:{best:{score:2}},fixed_anatomy:{best:{score:2.2}},identifiability:'Competing explanations remain; no anatomy validation'}};
...
test('optional source flow presents alternatives and requires frozen prediction confirmation before scoring',async({page})=>{
 ...
 await page.route('**/api/source/score',r=>{actions.push('score');state={...state,score:{status:'succeeded',result:{status:'model_mismatch',score:4.2,score_excluding_pitch:1.3,requested_f0_hz:180,simulated_f0_hz:171.5,model_updated:false}}};return r.fulfill({status:202,json:{accepted:true}})});
 ...
 const hypotheses=panel.getByRole('table').first();await expect(hypotheses.getByRole('columnheader',{name:'Requested → simulated F0 (Hz)'})).toBeVisible();await expect(hypotheses.getByRole('columnheader',{name:'Without pitch term'})).toBeVisible();
 await expect(hypotheses.getByRole('row').nth(2)).toContainText('two_mass');await expect(panel).toContainText('Source family: Unavailable. Source policy: software-test.');await expect(hypotheses.getByRole('row').nth(2)).toContainText('180.0 → 171.5');await expect(hypotheses.getByRole('row').nth(2)).toContainText('0.900');
 await expect(panel).toContainText('The two-mass model can miss the requested pitch');await hypotheses.scrollIntoViewIfNeeded();await page.screenshot({path:'test-results/source-inference-fit-f0.png'});
