...
test('optional source flow presents alternatives and requires frozen prediction confirmation before scoring',async({page})=>{
 ...
 await expect(hypotheses.getByRole('row').nth(2)).toContainText('two_mass');await expect(hypotheses.getByRole('row').nth(2)).toContainText('180.0 → 171.5');await expect(hypotheses.getByRole('row').nth(2)).toContainText('0.900');
